...
# Get all biosample from library
BioSou, BioSouAll = GetItemComp('Biosample', 'source', LibBio)
...
Val.append(sum(200 > i > 99 for i in LabOccO))
Val.append(sum(500 > i > 199 for i in LabOccO))
Val.append(sum(1000 > i > 499 for i in LabOccO))
Val.append(sum(i > 999 for i in LabOccO))
Label.extend(['>20', '>50', '>100', '>200', '>500', '>1000'])

indexes = np.arange(len(Label))
width = 1
plt.bar(indexes, Val, width)
plt.xticks(indexes + width * 0.5, Label)
plt.savefig('Figures/ExperimentSourceHist.ps')
