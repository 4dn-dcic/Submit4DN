...
for sheet in sheet_order:
    ...
    write_list = ['#use']
    write_list.extend(useful)
    write_list.append('#notuseful')
    write_list.extend(not_used)

    # write new sheets for ordering
    new_sheet = book_w.add_sheet(sheet)
    for r in range(len(write_list)):
        new_sheet.write(r, 0, write_list[r])
